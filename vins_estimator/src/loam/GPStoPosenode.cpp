...
void GPSHandler(const sensor_msgs::NavSatFixConstPtr& msg)
{
 ...
 if(GPS_status<2)
 {
	//std::cout <<"-------GPS_status= "<<GPS_status<< std::endl;
        ...
        //pose_stamped.header.frame_id = "/GPS_world";
	...
	pose_stamped.pose.position.z=tempz; 
        ...
        //GPSpath.header.frame_id = "/GPS_world";
	...
	//Align GPS world frame and Lidar world frame      //D
	static tf::TransformBroadcaster br;
	static tf::TransformListener listener;
	static double last_align_time = -1;


        /*static tf::Transform t_odom_gps = tf::Transform(tf::createQuaternionFromRPY(0, 0, 0), tf::Vector3(0, 0, 0));
        if (msg->header.stamp.toSec() - last_align_time > 1.0)
        {
            try
            {
                tf::StampedTransform trans_odom_gps;
		listener.waitForTransform("odom","GPS_world", ros::Time(0), ros::Duration(3.0));
                listener.lookupTransform("odom","GPS_world", ros::Time(0), trans_odom_gps);
		t_odom_gps=trans_odom_gps;
                last_align_time = msg->header.stamp.toSec();
            } 
            catch (tf::TransformException ex){}
        }
        br.sendTransform(tf::StampedTransform(t_odom_gps, msg->header.stamp, "odom", "GPS_world"));*/

      	//static tf::Transform t_odom_odom_inv = tf::Transform(tf::createQuaternionFromRPY(0, 3.141, 0), tf::Vector3(0, 0, 0));
	//br.sendTransform(tf::StampedTransform(t_odom_odom_inv, GPSpath.header.stamp, "odom", "odom_invert"));


  	//Write results to a file  //D
...
void GPSHandler_ppk(const sensor_msgs::NavSatFixConstPtr& msg)
{
  if(!use_ppk) return;
  if (msg->status.status < sensor_msgs::NavSatStatus::STATUS_FIX) 
  {
...
int main(int argc, char **argv)
{
    ...
    ros::Subscriber sub_GPS_ppk = nh.subscribe("/fix_ppk", 100, GPSHandler_ppk);        //O - PPK topic
